...
import { showMessage, Time, welcomeMessage } from './message.js';
import { Config, ModelList, ModelManager } from './model.js';
...
async function loadWidget(config: Config) {
  ...
  if (config.models && Array.isArray(config.models)) {
    logger.info(
      `使用配置中的自定义模型列表，共 ${config.models.length} 个模型`,
    );
  ...
  // 将模型管理器暴露到全局，方便外部访问
  (window as any).modelManager = model;
}
